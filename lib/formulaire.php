...
class Formulaire{
	...
	public function creerInputTexteDisable($unNom, $unId, $uneValue , $required , $placeholder , $pattern){
		$composant = "<input type = 'text' name = '" . $unNom . "' id = '" . $unId . "' ";
		if (!empty($uneValue)){
			$composant .= "value = '" . $uneValue . "' ";
		}
	...
	public function creerInputHidden($unNom, $unId, $uneValue , $required , $placeholder , $pattern){
	...
	public function creerFormulaire(){
		$this->formulaireToPrint = "<form target='_self' method = '" .  $this->method . "' ";
